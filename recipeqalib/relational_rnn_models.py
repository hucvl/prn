...
class RelationalMemory(nn.Module):
    ...
    def __init__(self, mem_slots, head_size, input_size, num_heads=1, num_blocks=1, forget_bias=1.,
                 ...
                 save_step_wise_attentions=False):
    ...
    def initial_state(self,ingredient_embeddings,memory_mask, trainable=False):
        """
        ...
        Returns:
          init_state: A truncated or padded matrix of size
            (batch_size, self.mem_slots, self.mem_size).
        """
        init_state = ingredient_embeddings
        count = torch.nonzero(memory_mask).size(0)
        # pad the matrix with zeros
        if self.mem_size >= self.mem_slots:
            init_state = init_state[:,0:count,:]
            dvc = init_state.get_device()
            if self.mem_slots > count:
                difference = self.mem_slots - count
                pad = torch.zeros((1, difference, self.mem_size)).cuda(dvc)
    ...
    def attend_over_memory(self, memory,memory_mask):
        """
        ...
        for _ in range(self.num_blocks):
            if self.save_step_wise_attentions:
                weights,attended_memory = self.multihead_attention(memory,memory_mask)
            else:

                attended_memory = self.multihead_attention(memory,memory_mask)
    ...
    def forward_step(self, inputs, memory, treat_input_as_matrix=False, memory_mask = None):
        """
        ...
        if treat_input_as_matrix:
            ...
            inputs_embed = torch.t(inputs_embed)
        ...
        if self.save_step_wise_attentions:
            weights,next_memory = self.attend_over_memory(memory_plus_input,memory_mask)
        else:

            next_memory = self.attend_over_memory(memory_plus_input,memory_mask)
        ...
        if not self.use_adaptive_softmax:
            ...
            logit = output_embed
        if self.save_step_wise_attentions:
            return weights,logit,next_memory
        return logit, next_memory

    def forward(self, inputs, memory, targets, require_logits=False,treat_input_as_matrix = False,memory_mask = None):
        # Starting each batch, we detach the hidden state from how it was previously produced.
        # If we didn't, the model would try backpropagating all the way to start of the dataset.
        memory = self.repackage_hidden(memory)
        dvc = memory_mask.get_device()  # for training with multi-gpu
        memory_mask = torch.cat([memory_mask,torch.zeros(1).cuda(dvc)])
        memory_mask = memory_mask.unsqueeze(0)
        memory_mask[0,self.mem_slots] = 1
        ...
        for idx_step in range(inputs.size(0)):
            if self.save_step_wise_attentions:
                weights,logit, memory = self.forward_step(inputs[idx_step, :],
                                              memory,
                                              treat_input_as_matrix=treat_input_as_matrix,
                                              memory_mask=memory_mask)
                logits.append(logit)
            else:
                logit, memory = self.forward_step(inputs[idx_step, :],
                                              memory,
                                              treat_input_as_matrix=treat_input_as_matrix,
                                              memory_mask=memory_mask)
                logits.append(logit)
        ...
        if self.save_step_wise_attentions:
            return self.weights_list,loss,memory
